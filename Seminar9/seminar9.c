...
void adaugaMasinaInArbore(Nod** radacina, Masina masinaNoua) {
    ...
    if((*radacina) != NULL){
        if(masinaNoua.id < (*radacina)->info.id) {
            ...
            (*radacina)->info.model = (char*)malloc(strlen(masinaNoua.model));
            strcpy((*radacina)->info.model, masinaNoua.model);

            (*radacina)->info.numeSofer = (char*)malloc(strlen(masinaNoua.numeSofer));
    ...
    //la celelalte grupe e diferit, s a facut inserare la dreapta, 34'
}
...
Masina getMasinaByID(Nod* radacina, int id) {
    ...
    if(id>radacina->info.id){
        ...
        m = radacina->info;
        m.numeSofer = (char*)malloc(strlen(radacina->info.numeSofer));
        strcpy(m.numeSofer,radacina->info.numeSofer);

        m.model = (char*)malloc(strlen(radacina->info.model));
