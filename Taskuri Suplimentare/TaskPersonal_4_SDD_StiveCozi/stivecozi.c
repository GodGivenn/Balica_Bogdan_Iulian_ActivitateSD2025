...
void pushStack(Nod** stiva, Masina masina) {
	/*
		1. Creez nod nou
		2. Pun masina în nod
		3. Leg noul nod la vechiul varf: nou->next = (*stiva)
		4. Fac noul nod sa fie varful: (*stiva) = nou
	*/
...
Masina popStack(Nod** stiva) {
	if ((*stiva) != NULL) { //1. Verific daca stiva nu e goala
		Masina nou = (*stiva)->info; //2. Salvez masina din varf
		Nod* prev = (*stiva); 
		(*stiva) = (*stiva)->next;
		free(prev); //4. Eliberez vechiul varf
		return nou; //5. Returnez masina salvata
	}
	Masina m;
	m.id = -1;
	return m;
...
Masina dequeue(ListaDubla* lista) {
	1. Verific dacă lista nu e goală
	2. Salvez mașina din primul nod
	3. Mută primul la următorul nod
	4. Dacă noul prim există, îi rup legătura prev
	5. Altfel, lista devine goală (și prim și ultim = NULL)
	6. Eliberez vechiul prim
	7. Returnez mașina salvată
}
*/
